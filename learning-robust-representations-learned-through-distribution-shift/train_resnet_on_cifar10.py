...
def train(args):
    ...
    dataloader_test, dataloader_train = util.vanilla_mnist_loaders(args, type='cifar10')

    buffer_model = util.load_ssl_model(args, device)
    ...
    if args.buffer_path == "none":
        ...
        model = ResNetRetrieval18(
                    retrieval_buffer=rep_buffer,
                    ...
                    no_labels_from_reps=args.no_labels_from_reps)
...
if __name__ == '__main__':
    ...
    wandb.init(project="resnet_retrieval", entity='dataset-generalization')
    wandb.config.update(run_args)
    wandb.run.name = f"resnet_cifar10_{run_args.name}"
    train(run_args)
