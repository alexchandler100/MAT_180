...
Many music today are written in 12-tone equal temperament
, with more than a single instrument (multiple tracks) and 
...
Outside of philosophy however, and constraining our music to be short piano pieces,
 we demonstrate that in a sense it can. Our neural network and generation
...
folder structure. The same function will return a dictionary `genres_dict` which
